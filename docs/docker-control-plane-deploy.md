...
- The public Bore deployment on the Hetzner VPS uses Traefik ACME `http-01`.
- Keep that public deployment on `http-01`.
- Do not add zone-wide DNS API credentials to the public Traefik deployment to support private RFC1918 targets.

If you need a private LAN hostname such as `s.bore.dk -> 192.168.1.200`, terminate TLS on that private host or network with a separate reverse proxy and use delegated ACME `dns-01` for that hostname only. See `docs/acme-challenge-policy.md`.
